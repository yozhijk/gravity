/**
    ...
    SceneGraph represents a set of nodes of several kinds each of which can have parameters of arbitrary type.
...
namespace Gravity
{
    ...
    */
    template <typename Key, typename NodeType, typename Parameter> class SceneGraph
    {
        public:
            ...
            */
            class Node
            {
                public:
                    ...
                    template <typename T> void SetValue(Key const& key, T&& value)
                    {
            ...
            */
            template <typename Func> struct FilteredCallback
            {
                ...
                template <typename... Args> void operator()(Node* node, Args&& ... args)
                {
            ...
            void DeleteNode(Node* node)
            {
                ...
                auto iter = std::find_if(m_nodes.cbegin(), m_nodes.cend(), [node](std::unique_ptr<Node> const& ptr) { return ptr.get() == node; });
    ...
    template <typename Key, typename NodeType, typename Parameter> std::ostream& operator << (std::ostream& out, typename SceneGraph<Key, NodeType, Parameter>::Node const& node)
    {
        // TODO: implement me
    ...
    template <typename Key, typename NodeType, typename Parameter> std::ostream& operator << (std::ostream& out, SceneGraph<Key, NodeType, Parameter> const& sg)
    {
        // TODO: implement me
